...
def main():
    ...
    ap.add_argument("--pattern", default="*.png,*.jpg")
    args = ap.parse_args()
    ...
    if args.file:
        ...
        pats = [s.strip() for s in args.pattern.split(",") if s.strip()]
        for p in pats:
            files += [str(x) for x in Path(args.dir).glob(p)]
    items = []
    for f in files:
        items.append({"file": Path(f).name, "result": client.upload_attachment(args.schema, args.id, args.field, f)})
